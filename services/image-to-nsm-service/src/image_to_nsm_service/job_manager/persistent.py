...
class PersistentJobManager:
    ...
    def create_job(self, input_filename: Optional[str] = None) -> JobRecord:
        ...
        self.append_log(job_id, "info", "Job created.")
    ...
    def update_job(self, job_id: str, **updates: Any) -> Optional[JobRecord]:
        ...
        if "input_image_bytes" in updates:
            ...
            artifact_updates["input_image"] = _to_artifact_row(stored)
        if "raw_output" in updates:
            ...
            if raw_output is not None:
                ...
                artifact_updates["raw_llm_output"] = _to_artifact_row(stored)
        if "normalized_output" in updates:
            ...
            if normalized is not None:
                ...
                artifact_updates["normalized_nsm"] = _to_artifact_row(stored)
        if "nsm" in updates:
            nsm = updates.pop("nsm")
            if nsm is not None:
                stored = self._artifacts.store_json(job_id, "final_nsm", nsm)
                artifact_updates["final_nsm"] = _to_artifact_row(stored)
        if "validation_report" in updates:
            ...
            if report is not None:
                ...
                artifact_updates["validation_report"] = _to_artifact_row(stored)
        ...
        if "status" in metadata_updates:
            self.append_log(job_id, "info", f"Status set to {metadata_updates['status']}.")
...
def _to_artifact_row(stored) -> ArtifactRow:
    return ArtifactRow(
